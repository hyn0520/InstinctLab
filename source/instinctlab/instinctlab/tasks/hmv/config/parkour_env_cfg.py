...
class SceneCfg(InteractiveSceneCfg):
    ...
    camera = NoisyGroupedRayCasterCameraCfg(
        prim_path="{ENV_REGEX_NS}/Robot/head_pitch_link",
        mesh_prim_paths=["/World/ground/", "/World/envs/env_.*/Robot/(?!.*head_pitch_link).*"],
    ...
    #     mesh_prim_paths=["/World/ground/","/World/envs/env_.*/Robot/(?!.*head_pitch_link).*",],
    #     attach_yaw_only=False,
    #     pattern_cfg=PinholeCameraPatternCfg.from_intrinsic_matrix(
    #         intrinsic_matrix=[
    #             25.580, 0.0, 31.383,
    #             0.0, 34.310, 22.524,
    #             0.0, 0.0, 1.0,
...
class G1Rewards:
    ...
    freeze_upper_body = RewTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg(
                "robot", joint_names=[".*_shoulder_.*", ".*_elbow_.*",  "waist_yaw_joint.*", "head_yaw_joint", "head_pitch_joint.*"]
            ),
        },
    )
